...
- [ ] **实体类设计** - 进行中（User已完成，其余待开发）
- [ ] **Repository接口** - 进行中（UserRepository已完成）
- [ ] **Service业务逻辑** - 进行中（UserService已完成）
- [ ] **业务Controller API** - 进行中（AuthController已完成登录/资料接口）
...
### 🔧 当前代码状态
- 前端已实现登录、首页（含购物车）、添加菜品、菜品详情、个人中心等页面，数据暂存微信本地存储
- 新增菜品与菜品详情支持编辑、删除、购物车添加等核心交互
- 后端已配置Spring Security + JWT，并提供登录、资料查询更新等认证接口
- 项目结构和类型定义完善，可继续对接真实后端API与订单流程
...
#### 5. 认证与安全模块
- ✅ **AuthController**：提供登录、用户资料查询与更新接口（使用模拟openid流程）
- ✅ **UserService & UserRepository**：实现用户的创建、更新、查询、去重校验
- ✅ **JWT工具与安全配置**：`JwtUtil`、`JwtAuthenticationFilter`、`SecurityConfig`已整合，实现基于JWT的无状态认证

### ❌ 待开发的核心功能

#### 1. 实体类层 (Entity)
- ✅ **User实体**：已完成（包含角色枚举与自动时间戳）
- ❌ **菜品与订单实体**：Dish, DishIngredient, DishStep, Order, OrderItem, DishRating

#### 2. 数据访问层 (Repository)
- ✅ **UserRepository**：已完成
- ❌ **其他Repository接口**：菜品、订单等CRUD待开发

#### 3. 业务逻辑层 (Service)
- ✅ **UserService**：完成用户创建、更新、查询逻辑
- ❌ **菜品与订单Service**：待开发（含菜品管理、订单流转、统计等）

#### 4. 控制器层 (Controller)
- ✅ **AuthController**：已提供登录与资料接口
- ❌ **业务Controller**：菜品管理、订单管理、统计API待实现
...
#### 3. 实体类与数据访问
- ✅ **用户相关模块**：User实体、UserRepository、UserService已完成基础功能
...
- ✅ **数据库连接池**：HikariCP配置
- ✅ **JPA配置**：Hibernate自动建表
- ✅ **安全配置**：Spring Security + JWT认证过滤器已接入
- ✅ **日志配置**：文件日志和调试级别
